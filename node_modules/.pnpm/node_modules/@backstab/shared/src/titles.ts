...
export function computeTitles(
  ...
  if (history.length === 0 || playerIds.length === 0) return [];
  ...
  const stats: Record<
    PlayerId,
    {
      ...
      timesGuilty: number;
      timesInnocent: number;
      appealsTriggered: number;
  ...
  for (const { verdict } of history) {
    ...
    if (stats[def]) {
      ...
      if (verdict.outcome === "GUILTY") stats[def].timesGuilty += 1;
      if (verdict.outcome === "INNOCENT") stats[def].timesInnocent += 1;
      if (verdict.appealed) stats[def].appealsTriggered += 1;
    }

    for (const [pid, delta] of Object.entries(verdict.xpDelta)) {
      if (!stats[pid]) continue;
      if (delta > 0) stats[pid].totalXpGained += delta;
      if (delta < 0) stats[pid].totalXpLost += Math.abs(delta);
    }
  }

  const titles: MatchTitle[] = [];
  ...
  const jurysDarling = pickMax(
    playerIds,
    (p) => (stats[p].timesDefended > 0 ? stats[p].timesInnocent / stats[p].timesDefended : 0),
    used,
  );
