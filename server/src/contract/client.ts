...
export async function createMatchOnContract(
  ...
  // ── 0. Bradbury path: txDataDecoded ──────────────────────────────────────
  if (client.chain?.id === 4221) {
    const td = r.txDataDecoded;
    if (typeof td === "number") {
      matchId = td;
    } else if (td && typeof td === "object" && typeof td.match_id === "number") {
      matchId = td.match_id;
    }
    console.log(`[contract] create_match Bradbury txDataDecoded: ${JSON.stringify(td)?.slice(0, 200)}`);
  }

  // ── 1. decoded_data (clean path) ─────────────────────────────────────────
  if (matchId === undefined && typeof r.decoded_data === "number") {
    matchId = r.decoded_data;
  } else if (matchId === undefined && r.decoded_data && typeof r.decoded_data === "object" && typeof r.decoded_data.match_id === "number") {
...
export async function submitRoundToContract(params: {
  ...
  console.log(`[contract] submit_round tx=${hash} — polling (retries=400, interval=5s)…`);
  const receipt = await client.waitForTransactionReceipt({
    hash,
    status: TransactionStatus.FINALIZED,
    interval: 5000,
  ...
  // ── 0. Bradbury path: txDataDecoded ──────────────────────────────────────
  if (client.chain?.id === 4221 && r.txDataDecoded && typeof r.txDataDecoded === "object") {
    result = r.txDataDecoded as ContractVerdict;
    console.log(`[contract] submit_round result from Bradbury txDataDecoded`);
  }

  // ── 1. decoded_data (clean path) ─────────────────────────────────────────
  if (!result && r.decoded_data && typeof r.decoded_data === "object") {
